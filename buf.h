 ...
 *      Sentinel value indicating the maximum enumeration value. Not used for logic.
 */
...
typedef enum status_code {
    ...
    BUSY = 4,               /**< The resource is busy and cannot process the request at this time. */
    UNAVAILABLE = 5,        /**< The resource is unavailable for use. */
    INVALID = 6,            /**< The operation was attempted with invalid input or configuration. */
...
typedef struct mpmc_queue {
    ...
    uint32_t capacity; // The capacity of the queue. Measured in units of "elements". Must be u32_max or less.
    ...
    overwrite_behavior_t overwrite_behavior; // Used to control behavior of the queue when full. If OVERWRITE, overwrite the oldest entries in the queue. If FAIL, return failure.
} mpmc_queue_t;
 ...
 * @param[out] entry Pointer to the location where the dequeued entry will be stored.
 *             User must allocate memory for this pointer
 ...
 * overwrite policy.
 *
 * @param[out] queue Pointer to the queue structure to be initialized. - If NULL, a new buffer is malloc()ed.
 ...
 *        - Setting this invalid or to ENUM_MAX results in choosing the FAIL operation
 * @return A status code indicating the result of the operation:
